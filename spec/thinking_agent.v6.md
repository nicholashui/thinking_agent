...
**Change policy:** v6 supersedes v5 and is SELF-CONTAINED BY CONSTRUCTION. Part I is the complete v5 specification, verbatim (all 33 sections). Part II is the complete v6 self-curriculum layer, including the INSTANTIATED ROUTER CONFIGURATION — all 212 historical strategy references (§II.2.6) with situation signatures, routed styles, historical outcomes, lessons, and artifact references — so the agent routes dynamically to the best human thinking model for any situation from recorded success/failure evidence. No external document is required. Companion executable artifacts: `validation/harness.py`, `validation/style_router.py`, `human_thinking_models.json`, `style_routing_kb.json`, `v5/test_cases/`, `v5/traces/`.
